...
export const Details = (props) => {
  ...
  useEffect(() => {
    ...
    fetch(
      `http://localhost:3000/filmes/${movieId}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className="movie-page">
        ...
        <div className="main-container">
          ...
          <h1 className="product-title"> {movieDetails.original_title}</h1>
    ...
    <div></div>
            ...
            {movieDetails?.credits?.cast.map((actor) => (
                       ...
                       <div class="card-body">
                         <h5 class="card-title">Nome: {actor.name}</h5>
                         <p class="card-text">Personagem: {actor.character}</p>
